...
ykn = [zeros(1,23); zeros(1,23); zeros(1,23)]; %Unfiltered noisy measurements
estimate = zeros(3,23); %EKF results
...
%    estError(i+1, :) = [estd_1 estd_2 estd_3];
    ...
    distance(i+1) = norm([xpos(i+1); ypos(i+1)]-[xpos_(i+1); ypos_(i+1)]); % Distance to ideal (noiseless) path using unfiltered estimate
    delta(i+1) = norm([yk(1); yk(2)] - [xpos(i+1); ypos(i+1)]); % Magnitude of measurement noise in x and y
    estDistance(i+1) = norm([xk_plus(1); xk_plus(2)]-[xpos(i+1); ypos(i+1)]); % Distance from estimated location to noisy (actual) location
end
